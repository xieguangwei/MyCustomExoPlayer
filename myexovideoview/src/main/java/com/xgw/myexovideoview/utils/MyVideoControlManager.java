...
import com.xgw.myexovideoview.videogesture.BrightnessHelper;
...
import com.xgw.myexovideoview.videogesture.VideoGestureListener;
...
public class MyVideoControlManager implements VideoGestureListener {
    ...
    private MyExoVideoView mVideoView;

    public void bindView(@NonNull Activity mActivity, @NonNull MyExoVideoView mVideoView, @NonNull VideoGestureLayout vgl, @NonNull ShowControlLayout scl) {
        this.vgl = vgl;
        this.scl = scl;
    ...
    public void onFF_REWGesture(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        if (mVideoView == null || mVideoView.getPlayer() == null || !mVideoView.isPlaying() || vgl == null) {
            return;
        }

        float duration = mVideoView.getPlayer().getDuration()/1000;
...
//            scl.setImageResource(R.drawable.ff);
    ...
    public void onDown(MotionEvent e) {
        ...
        float duration = mVideoView.getPlayer().getDuration();
        float currentPlayBackTime = mVideoView.getPlayer().getCurrentPosition();
        if (duration != 0) {
    ...
    public void onEndFF_REW(MotionEvent e) {
        ...
        KLog.e("设置进度为" + newPlaybackTime);
        mVideoView.seek((long) newPlaybackTime * 1000);
    }
}
